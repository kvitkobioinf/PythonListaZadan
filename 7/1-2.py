...
class addToLeaderboards(object):
    def __init__(self, master, text):
        ...
        self.e = Entry(top)
        self.b = Button(top, text="Enter name to add to leaderboards", command = self.cleanup)
    ...
    def sql(self, name):
        ...
        c.execute("INSERT INTO leaderboards VALUES ('" + d + "', '" + t + "', '" + name + "', '" + self.text + "')")
...
class mainWindow(object):
    ...
    def generate(self):
        ...
        self.stuffToScreen.append(Button(self.master, text="Sprawdź wynik", command=self.check))
        self.stuffToScreen.append(Button(self.master, text="Leaderboards", command=self.leaderboards))

        amountOfCollumns = 6

        for x in range(len(self.otherStuff)):
            if not x % amountOfCollumns:
                self.otherStuff[x].grid(row=math.floor(x / amountOfCollumns) * amountOfCollumns, column=0)
            else:
                self.otherStuff[x].grid(row=math.floor(x / amountOfCollumns) * amountOfCollumns, column=x % amountOfCollumns)

        for i in range(len(self.stuffToScreen)):
            x = len(self.otherStuff) + x % amountOfCollumns + 1 + i
            self.stuffToScreen[i].grid(row=math.floor(x / amountOfCollumns) * amountOfCollumns + 1, column=amountOfCollumns - i + 1)
